...
export function createGame(numPlayers: number, random: () => number, rules: Rules = CLASSIC_RULES): GameState {
  ...
  // Flip the first discard. Only a number card may open the round: an action or
  // wild card is buried at the bottom and the next card flipped instead, so the
  // round always starts from a clean position with seat 0 to act.
